...
$Session = "oralpath-stage2"
$ColabAuth = "oauth2"
$KaggleWin = Join-Path $HOME ".kaggle\kaggle.json"
$HfTokenWin = Join-Path $env:TEMP "oralpath_hf_token.secret"
$FullFlagWin = Join-Path $env:TEMP "oralpath_full_stage2.flag"
$FullEpochsWin = Join-Path $env:TEMP "oralpath_full_stage2_epochs.txt"
$ChunkFlagWin = Join-Path $env:TEMP "oralpath_stage2_chunk.flag"
$ResumeStateWin = Join-Path $RepoWin "model\evaluation\reports\colab_cli\stage2_last.pt"
...
if ($FullTraining) {
    ...
    try {
        ...
        Invoke-WslColab "source ~/.local/bin/env && colab --auth=$ColabAuth upload -s $Session '$FullFlagWsl' /content/full_stage2.flag"
    } finally {
    ...
    if ($FullEpochs -gt 0) {
        ...
        try {
            ...
            Invoke-WslColab "source ~/.local/bin/env && colab --auth=$ColabAuth upload -s $Session '$FullEpochsWsl' /content/full_stage2_epochs.txt"
        } finally {
    ...
    if (Test-Path $ResumeStateWin) {
        ...
        Write-Host "[5/7] Uploading resumable Stage 2 state..."
        Invoke-WslColab "source ~/.local/bin/env && colab --auth=$ColabAuth upload -s $Session '$ResumeStateWsl' /content/stage2_resume.pt"
    }
    if ($ChunkMode) {
        ...
        try {
            ...
            Invoke-WslColab "source ~/.local/bin/env && colab --auth=$ColabAuth upload -s $Session '$ChunkFlagWsl' /content/stage2_chunk_mode.flag"
        } finally {
...
Write-Host "[6/7] Running Stage 2 remote job..."
$PreviousErrorActionPreference = $ErrorActionPreference
$ErrorActionPreference = "Continue"
try {
    wsl -d Ubuntu -- bash -lc "source ~/.local/bin/env && cd '$RepoWsl' && colab --auth=$ColabAuth exec -s $Session --timeout 7200 -f scripts/colab_cli_stage2_job.py"
...
wsl -d Ubuntu -- bash -lc "source ~/.local/bin/env && colab --auth=$ColabAuth download -s $Session /content/oralpath/model/training/stage2_grading/logs/stage2_report.json '$RepoWsl/model/evaluation/reports/colab_cli/stage2_report.json' || true"
wsl -d Ubuntu -- bash -lc "source ~/.local/bin/env && colab --auth=$ColabAuth download -s $Session /content/oralpath/model/training/stage2_grading/checkpoints/stage2_last.pt '$RepoWsl/model/evaluation/reports/colab_cli/stage2_last.pt' || true"
if ($DownloadBest) {
    wsl -d Ubuntu -- bash -lc "source ~/.local/bin/env && colab --auth=$ColabAuth download -s $Session /content/oralpath/model/training/stage2_grading/checkpoints/stage2_best.pt '$RepoWsl/model/evaluation/reports/colab_cli/stage2_best.pt' || true"
} else {
    Write-Host "[7/7] Skipping full stage2_best.pt download. Use -DownloadBest when needed."
}
